  ...
  const { data: rows = [], isLoading } = useQuery<Row[]>({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .select(
          "id, row_index, item_name_raw, barcode, system_boxes, system_strips, system_units, system_quantity_raw, inventory_counts!left(phys_boxes, phys_strips, phys_units, status, is_current)",
        )
        .eq("session_id", id)
  ...
  return (
    <div className="space-y-2 mt-2">
      {rows.map((r) => {
        ...
        const d = phys ? diffTriple(sys, phys) : null;
        return (
